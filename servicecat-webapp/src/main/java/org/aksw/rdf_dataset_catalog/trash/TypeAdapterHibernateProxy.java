 ...
 */
public class TypeAdapterHibernateProxy extends TypeAdapter<HibernateProxy> {

    public static final TypeAdapterFactory FACTORY = new TypeAdapterFactory() {
        @Override
        @SuppressWarnings("unchecked")
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            return (HibernateProxy.class.isAssignableFrom(type.getRawType()) ? (TypeAdapter<T>) new TypeAdapterHibernateProxy(
                    gson) : null);
        }
    };
    private final Gson context;

    private TypeAdapterHibernateProxy(Gson context) {
